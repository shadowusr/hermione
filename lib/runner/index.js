...
const debug = require('debug')(`hermione:master:runner`);
...
module.exports = class MainRunner extends Runner {
    constructor(config, interceptors) {
        ...
        this._running = new PromiseGroup('hermione:master:runner:running');
        this._runned = false;
    ...
    init() {
        ...
        debug(`register ${this._config.system.workers} workers`);
    ...
    addTestToRun(test, browserId) {
        debug(`add test to run, test: ${test.fullTitle()}, browserId: ${browserId}`);

        if (!this._isRunning() || this._running.isFulfilled()) {
            debug('do not add test, it is already fulfilled');
            return false;
        }
