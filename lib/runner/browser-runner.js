...
const debug = require('debug')(`hermione:master:browser-runner`);
...
module.exports = class BrowserRunner extends Runner {
    constructor(browserId, config, browserPool, workers) {
        ...
        this._running = new PromiseGroup('hermione:master:browser-runner:running');
    }
    ...
    addTestToRun(test) {
        debug(`add test to run, test: ${test.fullTitle()}`);
