...
const debug = require('debug')(`hermione:worker:existing-browser`);
...
module.exports = class ExistingBrowser extends Browser {
    ...
    _takeScreenshot() {
        debug(`try to take screenshot for sessionId: ${this.sessionId}`);
        return this._session.screenshot().then(({value}) => value);
    }

    _addCommands() {
        debug(`add custom commands`);
    ...
    async init(sessionId, calibrator) {
        ...
        await this._performCalibration(calibrator, sessionId);
        await this._buildClientScripts(sessionId);

        debug(`session is completely inited: ${sessionId}`);

        return this;
    }

    async reinit(sessionId) {
        debug(`reinit already existed browser with sessionId: ${sessionId}`);
        await this._prepareSession(sessionId);
    ...
    async _prepareSession(sessionId) {
        debug(`prepare session: ${sessionId}`);
    ...
    _performCalibration(calibrator, sessionId) {
        debug(`perform calibration: ${sessionId}`);
    ...
    _buildClientScripts(sessionId) {
        debug(`build client scripts: ${sessionId}`);
    ...
    markAsBroken() {
        debug(`mark session as broken, sessionId: ${this.sessionId}`);
    ...
    quit() {
        debug(`quit from sessionId: ${this.sessionId}`);
        this.sessionId = null;
    ...
    open(url) {
        debug(`before open url for session: ${this.session}, url: ${url}`);
